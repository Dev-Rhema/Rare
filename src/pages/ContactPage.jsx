...
export default function ContactPage() {
  return (
    <>
      ...
      <Section>
        <div className="grid items-start gap-8 lg:grid-cols-[1.6fr_1fr]">
          ...
          <aside className="space-y-4">
            {CONTACT_CHANNELS.map((channel) => <InfoCard key={channel.title} {...channel} />)}
            <Card variant="blue" className="p-6">
              <h3 className="text-xl font-extrabold">{SELL_CTA.title}</h3>
              <p className="mt-2 text-sm leading-relaxed text-white/80">{SELL_CTA.description}</p>
              <p className="mt-4 inline-block rounded-lg bg-brand-yellow px-3 py-1.5 text-xs font-semibold text-ink">{SELL_CTA.label}</p>
            </Card>
          </aside>
        </div>
      </Section>
      ...
      <Section tone="yellow">
        <SectionHeader title="Before you write in" description="The questions we answer most often." />
        <FaqList items={FAQS} />
      </Section>
    </>
  );
